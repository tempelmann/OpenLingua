...
Inherits Canvas
	...
	#tag Method, Flags = &h0
		Sub ResizingWindow(w as Window, finished as Boolean)
		  // This will adjust the splitter so that it keeps it relative position in the window
		  // when the window gets resized.
		  //
		  // Call this during and at end of a Window's Resizing (finished=false) and
		  // Resized events (finished=true).
		  // Must also call once (finished=true) from Window.Open!
		  
		  if mSplitterRatio <> 0 then
		    if IsVertical then
		      me.Reposition mSplitterRatio * (w.Width-me.Width) - me.Width
		    else
		      me.Reposition mSplitterRatio * (w.Height-me.Height) - me.Height
		    end
		  end if
		  if finished then
		    if IsVertical then
		      mSplitterRatio = (me.Left+me.Width) / (w.Width-me.Width)
		    else
		      mSplitterRatio = (me.Top+me.Height) / (w.Height-me.Height)
		    end
		  end
		End Sub
	#tag EndMethod
	...
	#tag Property, Flags = &h21
		Private mSplitterRatio As Double
	#tag EndProperty
